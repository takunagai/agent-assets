...
2. **dry-run でプレビュー**
   ...
   ```
   - 出力（対象セッション / 残すフォーク元 / **フォーク系図** / 削除対象の一覧）をそのままユーザーに見せる。
   - プレビューには「==== フォーク系図 ====」が含まれる。削除対象の家系を ASCII ツリーで示す（✗=削除対象 / ⚠=削除対象の子孫 / ●=大元 / ○=残るノード）。プロジェクト全体のフォーク状況を俯瞰したいときは `--tree-all` を付ける。
   - 系図のあとに **⚠（子フォーク）WARNING** が出たら、その意味（削除対象に子孫フォークがあり、消すと子の `forkedFrom` 参照先が失われる。ただし子の transcript 自体は残り、削除もブロックされない）をユーザーに伝え、**先に子を消すか・このまま進めるか**の判断を仰ぐ。
