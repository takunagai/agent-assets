...
- **フォーク系図を可視化** — プレビューに削除対象の家系を ASCII ツリーで表示します（大元・削除対象・その子孫・兄弟の関係が一目で分かる）。削除対象に子孫フォークがある場合は警告します（削除はブロックしません）。
...
# 4) 系図をプロジェクト全体（全家系）で表示（他の動作は不変。dry-run とも --apply とも併用可）
~/.claude/skills/cc-fork-delete/delete-fork.sh <sid> --tree-all
```
...
方式             : ゴミ箱退避（復元可）
==== フォーク系図 ====
● 9f8e7d6c (大元・残す) 2026-07-10 14:22 312行
├─ ✗ 1a2b3c4d [削除対象] 2026-07-12 09:15 84行
│   └─ ⚠ 77c0de12 (削除対象の子孫・残る) 2026-07-12 10:03 40行
└─ ○ 5b4a3c2d (兄弟・残す) 2026-07-13 18:40 120行

WARNING: 削除対象には子フォークが 1 件あります。
         このまま削除すると、子フォークの forkedFrom 参照先（親）が失われます。
         子フォークの transcript 自体は残りますが、系譜がたどれなくなります。
         子フォークも消したい場合は、各 sid を指定してこのスキルを再実行してください。
         （削除自体は中止しません）
--- セッション実体（削除/退避）---
...
> 上の系図は削除対象（`✗`）の家系だけを描いています。マーカーの意味は `✗`=削除対象 / `⚠`=削除対象の子孫（残るが系譜が切れる）/ `●`=大元（根）/ `○`=その他の残るノード。各行は「sid 先頭 8 桁 ─ 役割注記 ─ 最終更新日時 ─ 行数 ─ セッション名（取得できた場合）」の順です。子は最終更新日時の昇順で並びます。

---

## フォーク系図（プレビュー内）

dry-run・`--apply` のどちらでも、削除対象の家系を ASCII ツリーで表示します。プロジェクト dir 直下の transcript（`*.jsonl`）の `forkedFrom` から親子関係を再構築したもので、削除で「何が消え・何が残り・誰が孤立するか」を実行前に俯瞰できます。

| マーカー | 意味 |
|:---:|------|
| `✗` | 削除対象 |
| `⚠` | 削除対象の子孫（削除しても transcript は残るが、親への系譜が切れる） |
| `●` | 大元（根）。`forkedFrom` を持たない、または親 transcript が欠落して遡れない先頭 |
| `○` | その他の残るノード（兄弟・先祖など） |

- **既定** は削除対象の家系（根から下の全子孫）だけを描きます。プロジェクトに複数の独立した家系があっても、関係する家系だけに絞られます。
- **`--tree-all`** を付けると、プロジェクト全体の全家系（全ルート）を描きます。フォークが溜まって全体像を把握したいときに使います。
- 親 transcript が見つからないフォーク（元セッションを先に消した等）は、そこで系図を打ち切り「親 transcript 欠落」と注記します。系図の構築に失敗しても削除フロー本体は止まりません（`（系図を構築できませんでした）` と 1 行出して続行）。
- サブディレクトリ（subagents 等）の `*.jsonl` は系図の対象外です。プロジェクト dir 直下のみを走査します。

### 子フォーク警告（削除はブロックしない）

削除対象に子孫フォークがある場合、系図の直後に WARNING を出します。伝える内容は次の 3 点です。

- 子フォークが何件あるか。
- 削除すると子の `forkedFrom` 参照先（親）が失われること（ただし**子の transcript 自体は残る**）。
- 子も消したい場合は、各 sid を指定してこのスキルを再実行すればよいこと。

**この警告は注意喚起のみで、削除自体は中止しません。** 子を残したまま親フォークだけ消すのは正当な操作なので、判断はユーザーに委ねられます。
...
- 14 ケース / 71 アサーションをカバー（dry-run、dict 形式 `forkedFrom`、UUID 不正、大元保護、実行中保護、退避、完全削除、Vault 無し環境、フォーク系図の描画、子フォーク警告、`--tree-all`、親 transcript 欠落時の続行など）。詳細は `tests/README.md` を参照。
